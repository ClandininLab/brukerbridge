...
def tiff_to_nii(xml_file):
    ...
    for channel in range(num_channels):
        ...
        if isMultiPageTiff and (isVolumeSeries is False):
            ...
            print(
                "isMultiPageTiff is {} / isVolumeSeries is {}".format(
                    isMultiPageTiff, isVolumeSeries
                )
            )
            ...
            for i in range(num_timepoints):
                ...
                if isMultiPageTiff:
                    ...
                    fullfile = os.path.join(data_dir, filename)
                    page = (
                        int(files[channel].get("page")) - 1
                    )  # page number -> array index
                ...
                if root.tag == "PVScan":
                    ...
                    for item in os.listdir(directory):
                        if item.endswith(".nii"):
                            print(
                                "skipping nii containing folder: {}".format(directory)
                            )
                            break
                    else:
